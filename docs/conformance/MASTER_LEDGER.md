...
| A-02 | Five channels on stable routes: `/photography`, `/motion`, `/stories`, `/modeling`, `/x` | §1.4             | `not-started` | `app/(site)/`               | —        | —                                                                |
| A-03 | Writing and modeling display labels remain CMS-configurable; no working name in a URL    | §1.4.1, ADR-0004 | `not-started` | `channelSettings` global    | —        | —                                                                |
...
| ID   | Requirement                                                                            | Source      | Status        | Implementation             | Evidence | Open issue                                 |
| ---- | -------------------------------------------------------------------------------------- | ----------- | ------------- | -------------------------- | -------- | ------------------------------------------ |
| B-01 | Publication color tokens and five channel theme contracts                              | §4.2        | `not-started` | `styles/tokens.css`        | —        | —                                          |
...
| B-05 | Three type roles: display serif, neutral grotesk, monospace metadata                   | §4.3        | `not-started` | `styles/typography.css`    | —        | Licensed faces pending brand approval      |
| B-06 | Self-hosted subset WOFF2; preload only above-the-fold faces                            | §4.3, §11.5 | `not-started` | `next/font`                | —        | —                                          |
| B-07 | Responsive type scale with fluid clamps                                                | §4.3        | `not-started` | `styles/typography.css`    | —        | —                                          |
| B-08 | 12/8/4-column responsive editorial grid with defined gutters and margins               | §4.4        | `not-started` | `styles/tokens.css`        | —        | —                                          |
| B-09 | Grid-breaking permitted only for a defined editorial reason                            | §4.4        | `not-started` | Block renderer             | —        | —                                          |
| B-10 | Mobile composition designed, not mechanically collapsed                                | §4.4, §15.5 | `not-started` | —                          | —        | —                                          |
...
| ID   | Requirement                                                                                    | Source         | Status        | Implementation                         | Evidence | Open issue                          |
| ---- | ---------------------------------------------------------------------------------------------- | -------------- | ------------- | -------------------------------------- | -------- | ----------------------------------- |
| C-01 | Motion tokens; no routine interaction exceeds ~700 ms                                          | §5.2           | `not-started` | `lib/motion/tokens.ts`                 | —        | —                                   |
| C-02 | Four capability tiers with automatic downgrade                                                 | §5.3           | `not-started` | `lib/capabilities/`                    | —        | —                                   |
...
| C-14 | Reduced-motion contract honored by every module; user preference overrides system              | §5.7           | `not-started` | `lib/motion/preference.ts`             | —        | —                                   |
...
| ID   | Requirement                                                           | Source         | Status        | Implementation                        | Evidence                  | Open issue                       |
| ---- | --------------------------------------------------------------------- | -------------- | ------------- | ------------------------------------- | ------------------------- | -------------------------------- |
| D-01 | Next.js App Router, server components by default                      | §6.2, §6.6     | `in-progress` | `app/`                                | —                         | —                                |
| D-02 | Payload CMS embedded in the same application                          | §6.2           | `not-started` | `app/(payload)/`, `payload.config.ts` | —                         | —                                |
| D-03 | PostgreSQL with migrations committed and run in CI                    | §6.2           | `in-progress` | `payload/migrations/`                 | Local Postgres 17 running | Managed provider not provisioned |
...
| D-09 | Repository structure matches the specified layout                     | §6.5           | `in-progress` | —                                     | Directory tree            | —                                |
...
| ID   | Requirement                                                                                                                | Source | Status        | Implementation                      | Evidence | Open issue                   |
| ---- | -------------------------------------------------------------------------------------------------------------------------- | ------ | ------------- | ----------------------------------- | -------- | ---------------------------- |
| E-01 | Twelve collections: stories, media, series, issues, tags, people, places, partners, authors, users, redirects, submissions | §7.1   | `not-started` | `payload/collections/`              | —        | —                            |
| E-02 | Seven globals including `channelSettings` and `disclosureSettings`                                                         | §7.2   | `not-started` | `payload/globals/`                  | —        | —                            |
| E-03 | Story schema: identity, dates, lead, body, relationships, credits/rights, SEO                                              | §7.3   | `not-started` | `payload/collections/Stories.ts`    | —        | —                            |
| E-04 | Calculated fields: reading time, aspect, search document, content hash, validation report                                  | §7.3   | `not-started` | `payload/hooks/`                    | —        | —                            |
| E-05 | Media schema: technical, editorial, and rights facets                                                                      | §7.4   | `not-started` | `payload/collections/Media.ts`      | —        | —                            |
| E-06 | Media without approved rights cannot be published except by a logged admin override                                        | §7.4   | `not-started` | `payload/hooks/rights.ts`           | —        | **Release blocker if unmet** |
| E-07 | Block library across text, image, motion, and relationship categories                                                      | §7.5   | `not-started` | `payload/blocks/`                   | —        | —                            |
| E-08 | Five editorial roles with least-privilege access control                                                                   | §7.6   | `not-started` | `payload/access/`                   | —        | —                            |
| E-09 | Publish validation blocks on missing required fields                                                                       | §7.7   | `not-started` | `payload/hooks/validate-publish.ts` | —        | —                            |
| E-10 | Live preview at 5 viewport presets; drafts never publicly reachable                                                        | §7.8   | `not-started` | `payload.config.ts`                 | —        | —                            |
| E-11 | Drafts, autosave, versions, scheduled publishing, rollback, retention policy                                               | §7.9   | `not-started` | —                                   | —        | —                            |
...
| F-10 | Image delivery: accurate `sizes`, eager LCP, lazy below fold, immutable derivative URLs | §8.8        | `not-started` | `components/media/EditorialImage.tsx`   | —        | —                                   |
...
| G-09 | CMS-managed 301 redirects; no published slug deleted without a decision   | §10.6       | `not-started` | `payload/collections/Redirects.ts` | —        | —          |
...
| I-02 | Semantic landmarks, one logical H1, skip link                                     | §12.2       | `not-started` | `app/(site)/layout.tsx`                | —        | —                            |
...
| J-05 | Admin not indexed; preview tokens short-lived and revocable                    | §13.3  | `in-progress` | `next.config.ts` X-Robots-Tag | —                         | —                                     |
...
| ID   | Requirement                                                                                                                                    | Source | Status        | Implementation       | Evidence | Open issue |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------- | -------------------- | -------- | ---------- |
| K-01 | Unit coverage of theme resolution, slugs, reading time, rights validation, related scoring, structured data, search parsing, motion preference | §14.1  | `not-started` | `tests/unit/`        | —        | —          |
