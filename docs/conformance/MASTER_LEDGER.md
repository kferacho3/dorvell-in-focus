...
| B-02 | Body copy meets WCAG contrast in every channel                                         | §4.2        | `verified`    | `styles/tokens.css`                     | `tests/a11y/axe.spec.ts` — contrast scanned on every channel incl. the dark one | Needs contrast test over all channel pairs |
| B-03 | Channel accent never carries meaning alone                                             | §4.2        | `verified`    | `ChannelNav`                            | `tests/e2e` — aria-current + rule + mono bullet, not colour alone               | —                                          |
...
| C-05 | Module A — Aperture Menu: reversible mid-animation, focus trap and return, Escape              | §5.5           | `verified`    | `components/navigation/ApertureMenu.tsx`         | `tests/e2e/aperture-menu.spec.ts` — 6 tests incl. mid-animation reversal | —                                   |
...
| C-07 | Module C — Shared Story Frame: stable key, instant fallback, back restores grid position       | §5.5           | `implemented` | `components/motion/SharedStoryFrame.tsx`         | `tests/e2e/navigation.spec.ts`                                           | —                                   |
...
| C-14 | Reduced-motion contract honored by every module; user preference overrides system              | §5.7           | `verified`    | `lib/motion/preference.ts` + `styles/motion.css` | Unit tests + reduced-motion Playwright project (18 tests)                | —                                   |
...
| D-03 | PostgreSQL with migrations committed and run in CI                    | §6.2           | `verified`    | `payload/migrations/`                 | 2 migrations; push:false so migrations are the only schema path | Managed provider not provisioned |
...
| D-06 | Search via Postgres FTS + trigram behind a `SearchProvider` interface | §6.8, ADR-0005 | `verified`    | `lib/search/`                         | Postgres FTS behind SearchProvider; 6 indexes created           | —                                |
...
| E-04 | Calculated fields: reading time, aspect, search document, content hash, validation report                                  | §7.3   | `verified`    | `payload/hooks/derive.ts`           | `tests/integration` — reading time, word count, search document, slug stability | —                            |
| E-05 | Media schema: technical, editorial, and rights facets                                                                      | §7.4   | `implemented` | `payload/collections/Media.ts`      | —                                                                               | —                            |
| E-06 | Media without approved rights cannot be published except by a logged admin override                                        | §7.4   | `verified`    | `payload/hooks/validate-publish.ts` | `tests/integration/publishing.test.ts` — 5 media-rights tests                   | **Release blocker if unmet** |
...
| E-08 | Five editorial roles with least-privilege access control                                                                   | §7.6   | `verified`    | `payload/access/`                   | `tests/integration` — drafts hidden, submissions forbidden                      | —                            |
| E-09 | Publish validation blocks on missing required fields                                                                       | §7.7   | `verified`    | `payload/hooks/validate-publish.ts` | `tests/integration/publishing.test.ts` — 6 publish-validation tests             | —                            |
...
| F-11 | Video delivery: adaptive HLS, immediate poster, captions, transcript, keyboard player   | §8.9        | `implemented` | `components/media/VideoPlayer.tsx`      | Native controls, preload=none, captions/transcript/chapters | Mux not provisioned (ADR-0007)      |
| F-12 | 9:16 work presented without a fake phone frame                                          | §8.9, §3.6  | `implemented` | `components/blocks/BlockRenderer.tsx`   | verticalVideoPair renders true 9:16, no device frame        | —                                   |

## G · Discovery and SEO

| ID   | Requirement                                                               | Source      | Status        | Implementation                                | Evidence                                            | Open issue |
| ---- | ------------------------------------------------------------------------- | ----------- | ------------- | --------------------------------------------- | --------------------------------------------------- | ---------- |
| G-01 | Archive with real pagination and crawlable, keyboard-operable fallback    | §3.10       | `implemented` | `app/(site)/archive/page.tsx`                 | Real pagination, stable URLs                        | —          |
| G-02 | Controlled tags with aliases, parent/child, status, and merge redirects   | §3.10       | `not-started` | `payload/collections/Tags.ts`                 | —                                                   | —          |
| G-03 | Search across titles, deks, body, captions, transcripts, and all entities | §3.10, §6.8 | `verified`    | `lib/search/`                                 | Weighted FTS + trigram verified against seeded data | —          |
| G-04 | Unique metadata on every indexable page                                   | §10.1       | `implemented` | `lib/seo/`                                    | Per-page metadata across 26 routes                  | —          |
| G-05 | JSON-LD generated from real content; no blanket schema stamping           | §10.2       | `implemented` | `lib/seo/structured-data.ts`                  | VideoObject omitted when required fields absent     | —          |
| G-06 | Segmented sitemaps excluding draft, thin, private, and noindex pages      | §10.3       | `implemented` | `app/sitemap.ts`                              | Serves 16 URLs; drafts and thin tags excluded       | —          |
| G-07 | Valid RSS/Atom with stable GUIDs                                          | §10.4       | `verified`    | `app/feed.xml/route.ts`                       | Well-formed XML confirmed; canonical-URL GUIDs      | —          |
...
| G-10 | Channel-aware dynamic OG art with a high-contrast fallback                | §10.7       | `verified`    | `app/(site)/story/[slug]/opengraph-image.tsx` | 1200x630 PNG rendered and visually checked          | —          |
...
| ID   | Requirement                                                                       | Source      | Status        | Implementation                           | Evidence                                                              | Open issue                   |
| ---- | --------------------------------------------------------------------------------- | ----------- | ------------- | ---------------------------------------- | --------------------------------------------------------------------- | ---------------------------- |
| I-01 | WCAG 2.2 AA baseline, tested beyond automated checks                              | §12.1       | `in-progress` | `tests/a11y/`                            | 13 axe tests, zero critical/serious. Manual AT pass still outstanding | —                            |
| I-02 | Semantic landmarks, one logical H1, skip link                                     | §12.2       | `verified`    | `components/layout/PublicationShell.tsx` | `tests/e2e` — skip link focuses main, one h1 per page                 | —                            |
...
| I-05 | Route changes announced to assistive technology                                   | §5.4, §12.2 | `implemented` | `components/layout/RouteAnnouncer.tsx`   | Polite live region announcing the new title                           | —                            |
| I-06 | Logical DOM order independent of visual grid placement                            | §12.2       | `not-started` | —                                        | —                                                                     | —                            |
| I-07 | Print stylesheet for stories                                                      | §12.2       | `implemented` | `styles/print.css`                       | —                                                                     | —                            |
...
| J-05 | Admin not indexed; preview tokens short-lived and revocable                    | §13.3  | `verified`    | `next.config.ts`             | `tests/e2e` asserts X-Robots-Tag noindex on /admin | —                                     |
...
| ID   | Requirement                                                                                                                                    | Source | Status        | Implementation       | Evidence                                                            | Open issue |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------- | -------------------- | ------------------------------------------------------------------- | ---------- |
| K-01 | Unit coverage of theme resolution, slugs, reading time, rights validation, related scoring, structured data, search parsing, motion preference | §14.1  | `verified`    | `tests/unit/`        | 43 unit tests passing                                               | —          |
| K-02 | Integration coverage of Payload hooks, draft/publish lifecycle, S3 adapter, search indexing, redirects, preview, scheduling                    | §14.1  | `verified`    | `tests/integration/` | 20 integration tests against real Postgres                          | —          |
| K-03 | E2E coverage including reduced motion, no-JS readability, and WebGL failure fallback                                                           | §14.1  | `verified`    | `tests/e2e/`         | 31 tests per project across desktop, mobile, no-JS, reduced-motion  | —          |
...
| K-07 | No release-blocking criterion is violated                                                                                                      | §14.5  | `in-progress` | —                    | No release blocker currently violated; domain clearance outstanding | —          |
